...
const getTasks = async (offset) => {
    let postSkip = 0
    if(offset){
        postSkip = (Number(offset) - 1) * MAX_ITEMS_PER_PAGE
    }
    console.log(postSkip);
    return Task.find()
               .skip(postSkip)
               .limit(MAX_ITEMS_PER_PAGE);
}

const getlatestTasks = async (ofs) => {
  const latest = await Task.find().sort({ $natural: -1 }).limit(ofs);
  return latest;
}
  /*if(offset){
      postSkip = (Number(offset) - 1) * MAX_ITEMS_PER_PAGE
...
/*
const getMoreTasks = async (offset) =>{
    const postSkip = (Number(offset) - 1) * MAX_ITEMS_PER_PAGE
    return Task.find()
               .skip(postSkip)
               .limit(MAX_ITEMS_PER_PAGE);
}
